...
function SearchForm({ query, onQueryChange, onSubmit, disabled }: { query: string; onQueryChange: (value: string) => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void; disabled: boolean }) {
  return <form className="search-form" onSubmit={onSubmit} aria-label="Search the knowledge base"><div className="search-input-row"><label className="sr-only" htmlFor="knowledge-query">Ask a question</label><input id="knowledge-query" className="search-input" value={query} disabled={disabled} onChange={(event) => onQueryChange(event.target.value)} placeholder="Ask about a policy, process, or source" /><button className="primary-button" type="submit" disabled={disabled}>Search evidence</button></div><div className="filter-row"><span className="source-chip">Authorization happens before evidence is rendered.</span></div></form>;
}

function ResultCard({ result, onOpenPreview, onAnswer, disabled }: { result: SearchResult; onOpenPreview: (result: SearchResult) => void; onAnswer: () => void; disabled: boolean }) {
  return <article className="result-card"><div className="card-heading"><div><p className="eyebrow">{sourceLabel(result.sourceType)}</p><h3>{result.title}</h3></div><StatusBadge state={lifecycleState(result.lifecycleState)} /></div><p>{result.safeSnippet}</p><div className="meta">{result.locator} · match {result.score.toFixed(2)}</div><div className="result-actions"><button className="text-button" type="button" disabled={disabled} onClick={() => onOpenPreview(result)}>Open safe preview</button><button className="text-button" type="button" disabled={disabled} onClick={onAnswer}>Use in answer</button></div></article>;
}

function ResultsView({ search, state, onOpenPreview, onAnswer, disabled }: { search: SearchResponse | null; state: UiState; onOpenPreview: (result: SearchResult) => void; onAnswer: () => void; disabled: boolean }) {
  ...
  if (!search || search.results.length === 0) return <section className="state-card"><StatusBadge state="no_accessible_context" /><p>No accessible context is available for this request.</p><button className="secondary-button" type="button" disabled={disabled} onClick={onAnswer}>Generate safe answer</button></section>;
  return <><div className="page-intro"><div><p className="eyebrow">Results / {search.queryId}</p><h1>Permitted evidence for your question</h1></div><StatusBadge state="fresh" /></div><div className="workspace-grid"><main className="panel"><div className="panel-heading"><div><p className="eyebrow">{search.results.length.toString().padStart(2, "0")} results</p><h2>Ranked after permission filtering</h2></div></div><div className="result-list">{search.results.map((result) => <ResultCard key={result.itemId} result={result} onOpenPreview={onOpenPreview} onAnswer={onAnswer} disabled={disabled} />)}</div></main><aside className="panel"><p className="eyebrow">Query summary</p><h2>Request trace</h2><p className="meta">{search.freshnessSummary.freshCount} fresh · {search.freshnessSummary.staleCount} stale</p><button className="primary-button" type="button" disabled={disabled} onClick={onAnswer}>Generate cited answer</button></aside></div></>;
}
...
export function SearchWorkbench({ initialPrincipal = "allowed-user" }: { initialPrincipal?: DemoPrincipal }) {
  ...
  return <><Topbar principal={principal} onPrincipalChange={changePrincipal} switching={switching} />{phase === "search" && <section className="hero"><div className="hero-copy"><p className="eyebrow">Search / evidence first</p><h1>Find the answer. See why it is safe.</h1><p>Only server-authorized context reaches the browser.</p><SearchForm query={query} onQueryChange={setQuery} onSubmit={runSearch} disabled={switching} /></div><aside className="hero-side"><StatusBadge state="fixture" /><button className="secondary-button" type="button" disabled={switching} onClick={() => void runSearch(undefined, canonicalQuestion)}>Use the seeded travel question</button></aside></section>}{phase === "results" && <ResultsView search={search} state={requestState} onOpenPreview={(result) => void openPreview(result)} onAnswer={() => void generateAnswer()} disabled={switching} />}{phase === "answer" && answer && <><div className="page-intro"><div><p className="eyebrow">Answer / {answer.answerId}</p><h1>Evidence you can inspect</h1><p>{query}</p></div><StatusBadge state={answer.status} /></div><div className="workspace-grid"><main><AnswerPanel answer={answer} onOpenCitation={(citation) => void openPreview(citation)} disabled={switching} /></main><SourcePreview preview={preview} onClose={() => setPreview(null)} /></div></>}<section className="panel"><p className="eyebrow">Interaction contract</p><div className="status-reference">{stateReference.map((state) => <StatusBadge key={state} state={state} />)}</div></section></>;
}
