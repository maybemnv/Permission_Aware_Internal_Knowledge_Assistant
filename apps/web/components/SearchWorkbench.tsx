...
function ResultsView({ search, state, onOpenPreview, onAnswer }: { search: SearchResponse | null; state: UiState; onOpenPreview: (result: SearchResult) => void; onAnswer: () => void }) {
  ...
  if (!search || search.results.length === 0) return <section className="state-card"><StatusBadge state="no_accessible_context" /><p>No accessible context is available for this request.</p></section>;
...
export function SearchWorkbench() {
  ...
  const runSearch = async (event?: FormEvent<HTMLFormElement>) => { event?.preventDefault(); setRequestState("loading"); setPhase("results"); setPreview(null); try { setSearch(await api.search(query)); setRequestState("fresh"); } catch { setSearch(null); setRequestState("failed"); } };
  ...
  return <><Topbar principal={principal} onPrincipalChange={changePrincipal} />{phase === "search" && <section className="hero"><div className="hero-copy"><p className="eyebrow">Search / evidence first</p><h1>Find the answer. See why it is safe.</h1><p>Only server-authorized context reaches the browser.</p><SearchForm query={query} onQueryChange={setQuery} onSubmit={runSearch} /></div><aside className="hero-side"><StatusBadge state="fixture" /><button className="secondary-button" type="button" onClick={() => { setQuery(canonicalQuestion); void runSearch(); }}>Use the seeded travel question</button></aside></section>}{phase === "results" && <>{requestState === "loading" && <div className="state-card" role="status"><StatusBadge state="loading" /></div>}<ResultsView search={search} state={requestState} onOpenPreview={(result) => void openPreview(result)} onAnswer={() => void generateAnswer()} /></>}{phase === "answer" && answer && <><div className="page-intro"><div><p className="eyebrow">Answer / {answer.answerId}</p><h1>Evidence you can inspect</h1><p>{query}</p></div><StatusBadge state={answer.status} /></div><div className="workspace-grid"><main><AnswerPanel answer={answer} onOpenCitation={(citation) => void openPreview(citation)} /></main><SourcePreview preview={preview} onClose={() => setPreview(null)} /></div></>}<section className="panel"><p className="eyebrow">Interaction contract</p><div className="status-reference">{stateReference.map((state) => <StatusBadge key={state} state={state} />)}</div></section></>;
}
